...
public class RobotData extends RunningData<RobotItems> implements DeviceSubject {
	...
	public String getDataForObservers(String robotIP){
		...
		if(!"".equals(jsonStr)){
			...
			info += "," + jsonObj.getString("slot10");
			info += "," + jsonObj.getString("slot11");
			info += "," + jsonObj.getString("slot12");
			info += "," + jsonObj.getString("slot13");
			info += "," + jsonObj.getString("slot14");
			info += "," + jsonObj.getString("slot15");
	...
	private String getRobotInfoJsonStr(String robotIP){
		...
		if(null != dt){
			...
			jsonObj.put("slot10", "" + (null!=dt.get(RobotItems.SLOT10)?dt.get(RobotItems.SLOT10):""));
			jsonObj.put("slot11", "" + (null!=dt.get(RobotItems.SLOT11)?dt.get(RobotItems.SLOT11):""));
			jsonObj.put("slot12", "" + (null!=dt.get(RobotItems.SLOT12)?dt.get(RobotItems.SLOT12):""));
			jsonObj.put("slot13", "" + (null!=dt.get(RobotItems.SLOT13)?dt.get(RobotItems.SLOT13):""));
			jsonObj.put("slot14", "" + (null!=dt.get(RobotItems.SLOT14)?dt.get(RobotItems.SLOT14):""));
			jsonObj.put("slot15", "" + (null!=dt.get(RobotItems.SLOT15)?dt.get(RobotItems.SLOT15):""));
			...
			if(MyFileUtils.getFilePassedDays(path)<=0.5){
				try {
					...
					if(null == restoreState || devState == restoreState){
						...
						setData(robotIP, RobotItems.SLOT10, jsonObj.getString("slot10"));
						setData(robotIP, RobotItems.SLOT11, jsonObj.getString("slot11"));
						setData(robotIP, RobotItems.SLOT12, jsonObj.getString("slot12"));
						setData(robotIP, RobotItems.SLOT13, jsonObj.getString("slot13"));
						setData(robotIP, RobotItems.SLOT14, jsonObj.getString("slot14"));
						setData(robotIP, RobotItems.SLOT15, jsonObj.getString("slot15"));
	...
	public void addMaterialInfo(String robotIP, String workpieceID, String slotID, DeviceState workpieceState){
		...
		System.out.println("at RobotData.addMaterialInfo  wpIDs:"+wpIDs+"    -----slotIDs:"+slotIDs);
...
//		System.out.println("at RobotData.getEmptySlots RobotEmptySlot :"+ Arrays.deepToString(emptySlots));
		return emptySlots;
	}
	...
	public void clearAllSlots(String robotIP){
//		for(int i=1; i<=9; i++){     //i<=9
		for(int i=1; i<=14; i++){     //i<=9
			setData(robotIP, getRobotItems(i), "");
		}
	}
	 ...
	 * @param robotIP
	 * @return position name where robot charging in
	 */
	public String getChargerPosition(String robotIP) {
		String chargerPos = "";
		LinkedHashMap<RobotItems, Object> data = getData(robotIP);
		if (null != data) {
			chargerPos = (String) data.get(RobotItems.TAGNAME);
		}
		return chargerPos;
	}
	...
	private RobotItems getRobotItems(int slotNo){
		...
		if(10 == slotNo) robotItem = RobotItems.SLOT10;
		
		if(11 == slotNo) robotItem = RobotItems.SLOT11;
		if(12 == slotNo) robotItem = RobotItems.SLOT12;
		if(13 == slotNo) robotItem = RobotItems.SLOT13;
		if(14 == slotNo) robotItem = RobotItems.SLOT14;
		if(15 == slotNo) robotItem = RobotItems.SLOT15;

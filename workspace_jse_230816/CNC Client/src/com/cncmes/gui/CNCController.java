				...
				btnLock1.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						if(btnLock1.isEnabled() && 1==e.getButton()){
							...
							if(!"".equals(errMsg)) JOptionPane.showMessageDialog(contentPanel, errMsg, "Operation Error", JOptionPane.ERROR_MESSAGE);
						}
					}
				});
				...
				btnUploadNcProgram.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						if(btnUploadNcProgram.isEnabled() && 1==e.getButton()){
							...
							if(!"".equals(errMsg)) JOptionPane.showMessageDialog(contentPanel, errMsg, "Upload Program Error", JOptionPane.ERROR_MESSAGE);
							...
							if(btnSendCommand.isEnabled() && 1 == arg0.getButton()){
								...
								if(null != cncCtrl){
									...
									case "uploadSubProgram":
										ok = cncCtrl.uploadSubProgram(ip,"","");
										break;
									case "downloadSubProgram":
										result = cncCtrl.downloadSubProgram(ip,"");
										ok = (result!=null)?true:false;
										break;
									...
									case "hfReleaseFixture":
										ok = cncCtrl.hfReleaseFixture(ip,1);
										break;
									case "hfResetDoor":

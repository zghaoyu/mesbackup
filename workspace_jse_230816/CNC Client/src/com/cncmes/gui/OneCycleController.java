		...
		if (null != ctrl) {
			...
			if (fromSlots.size() > 0) {
				for (int i = 0; i < fromSlots.size(); i++) {
					...
					ok = ctrl.pickAndPlace(ip, "input_rack_b_slot_" + fromSlots.get(i), "robot_pallet_a_slot_" + toSlots.get(i));
					if (!ok)
						break;
				}
			}
		...
		if (null != ctrl) {
			...
			if (fromSlots.size() > 0) {
				for (int i = 0; i < fromSlots.size(); i++) {
					...
					ok = ctrl.pickAndPlace(ip, "robot_pallet_a_slot_" + fromSlots.get(i), "input_rack_b_slot_" + toSlots.get(i));
					if (!ok)
						break;
				}
			}
		...
		if (null != ctrl) {
			...
			if (fromSlots.size() > 0) {
				for (int i = 0; i < fromSlots.size(); i++) {
					...
					ok = ctrl.pickAndPlace(ip, "robot_pallet_a_slot_" + fromSlots.get(i), "cnc_79_slot_" + toSlots.get(i));//change by Hui Zhi 
...
//			String machineIp = comboBoxIP.getSelectedItem().toString();
			...
			if (fromSlots.size() > 0) {
				for (int i = fromSlots.size() - 1 ; i >= 0; i--) {
					...
					ok = ctrl.pickAndPlace(ip, "cnc_79_slot_" + fromSlots.get(i), "robot_pallet_a_slot_" + toSlots.get(i));//change by Hui Zhi 
					if (!ok)
						break;
				}
			}

...
import com.cncmes.drv.RobotCMR;
...
public class RobotEthernetCmdConfig extends JDialog {
	...
	private static boolean loopingOperation = false;
	...
	private boolean executeCommand(boolean singleMode){
		...
		inParas.put("model", curRobotModel);
		if(singleMode) inParas.put("singleCmd", curRobotCommand);
		boolean success = false;
		if(curRobotModel.equals("R2D2")){
			success = RobotR2D2.getInstance().sendCommand(debugIP, curRobotOperation, inParas, null, curRobotTarget);
		}else if(curRobotModel.equals("CMR")){
			success = RobotCMR.getInstance().sendCommand(debugIP, curRobotOperation, inParas, null, curRobotTarget);
		}
		if(!success){
			if(singleMode){
			...
			}
			{
				...
				btnRun.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						if(1 == e.getButton()){
							if(!"subProgram".equals(curRobotOperation) &&
									!"retrySequence".equals(curRobotOperation)){
								ThreadUtils.Run(new ExecuteCommand(false));
							}else{
								JOptionPane.showMessageDialog(contentPanel, "This operation is not supported", "Operation Not Supported", JOptionPane.ERROR_MESSAGE);
							}
						}
					}
				});
		...
		}
		{
			...
			panelCenter.setLayout(new BorderLayout(0, 0));
			{
				...
				panelMainLeft.setLayout(gbl_panelMainLeft);
				{
					...
					lblAllCommands.addMouseListener(new MouseAdapter() {
						@Override
						public void mouseClicked(MouseEvent arg0) {
							if(2==arg0.getClickCount()){
								if(0==JOptionPane.showConfirmDialog(contentPanel, "Are you sure of reloading the settings from config file?", "Config Reloading", JOptionPane.YES_NO_OPTION)){
									XmlUtils.parseRobotEthernetCmdXml();
									setCurCommandParas();
								}
							}
				...
				btnExecuteCommand.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent arg0) {
						if(1 == arg0.getButton()){
							...
							ThreadUtils.Run(new ExecuteCommand(true));
						}
					}
				});
				{
	...
	class ExecuteCommand implements Runnable{
		...
		public void run() {
			...
			comboBoxRobotModel.setEditable(false);
			...
			comboBoxTarget.setEnabled(true);
			comboBoxRobotModel.setEditable(true);
		}
	}
}

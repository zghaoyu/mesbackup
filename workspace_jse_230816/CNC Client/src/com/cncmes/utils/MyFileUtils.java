...
public class MyFileUtils {
	...
	public static String[] getFileNames(String rootDir, String fileNameContains){
		String[] files = null;
		File temp = null;
		String tempStr = "";
		...
		if(fileList.length > 0){
			for(int i=0; i<fileList.length; i++){
				if(fileList[i].contains(fileNameContains)){
					temp = new File(rootDir + File.separator + fileList[i]);
					if(temp.isFile()){
						if("".equals(tempStr)){
							tempStr = fileList[i];
	...
	public static String chooseFile(Component parent, String titleText, String buttonText){
		String fileAbsPath = "";
		...
		jfc.showDialog(parent, buttonText);
		File file = jfc.getSelectedFile();
		if(file != null && file.isFile()) fileAbsPath = file.getAbsolutePath();

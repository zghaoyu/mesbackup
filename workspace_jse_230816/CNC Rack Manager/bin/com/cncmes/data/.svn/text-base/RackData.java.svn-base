...
public class RackData extends RunningData<RackItems> {
	 ...
	 * @param rackID can be "All" or real rackID
	 * @return null once there is no specified lineName or rackID, element value format is "lineName_rackID"
	 */
	 ...
	 */
	public boolean putWorkpieceOnRack(String lineName,String rackID,String workpieceID,String slotID){
		boolean ok = false;
		String[] emptySlots = null;
		...
		if(!"All".equals(lineName) && !"All".equals(rackID)){
			if(workpieceID.length() > 6){
				...
				if(null != emptySlots){
					if(null != slotID && !"".equals(slotID)) emptySlots[0] = slotID;
					...
					if(null == oriID){
						...
						wpData.setData(workpieceID, WorkpieceItems.CONVEYORID, rackID);
						wpData.setData(workpieceID, WorkpieceItems.CONVEYORSLOTNO, emptySlots[0]);
						ok = true;
					}
				}
			}
	...
	public void removeWorkpiece(String lineName,String rackID,String workpieceID){
		String[] mainKeys = getMainKey(lineName,rackID);
		if(null != mainKeys){
			...
			for(String mainKey:mainKeys){
				...
				if(null != rack){
					...
					for(int i=1; i<=capacity; i++){
						RackItems key = getRackItems(i);
						String id = (String) rack.get(key);
						if(null != id && workpieceID.equals(id)){
							rack.put(key, "");
							removed = true;
							break;
						}
	 ...
	 * @param lineName Once lineName="All" rackID will be "All" automatically
	 * @param rackID
	 * @return 0 once there is no not-empty slots for specified lineName and rackID
	 */
	...
	private String getRackInfoJsonStr(String lineName, String rackID){
		String jsonStr = "", materialID = "", spec = "";
		if("All".equals(lineName) || "All".equals(rackID)) return jsonStr;
		
		String mainKey = getMainKey(lineName, rackID)[0];
		LinkedHashMap<RackItems, Object> rack = getData(mainKey);
		if(null == rack) return jsonStr;
		...
		if(null != notEmptySlots && notEmptySlots.length > 0){
			for(String slot:notEmptySlots){
				slotNo = Integer.valueOf(slot);
				item = getRackItems(slotNo);
				materialID = ""+(null!=rack.get(item)?rack.get(item):"");
				jsonObj.put(slot, materialID);
				if("".equals(materialID)){
					jsonObj.put("state"+slot, "");
					jsonObj.put("spec"+slot, "");
				}else{
					spec = "" + wpData.getItemValStr(materialID,WorkpieceItems.PROCESS);
					spec += ";" + wpData.getItemValStr(materialID,WorkpieceItems.SURFACE);
					spec += ";" + wpData.getItemValStr(materialID,WorkpieceItems.MACHINETIME);
					spec += ";" + wpData.getItemValStr(materialID,WorkpieceItems.PROGRAM);
					spec += ";" + wpData.getItemValStr(materialID,WorkpieceItems.NCMODEL);
					
					jsonObj.put("state"+slot, ""+wpData.getWorkpieceState(materialID));
					jsonObj.put("spec"+slot, spec);
				}
			}
		}
		
		if(null != emptySlots && emptySlots.length > 0){
	...
	public void dumpRackInfo(String lineName, String rackID, String fileName){
		String[] keys = getMainKey(lineName, rackID);
		String[] tmp = null;
		String jsonStr = "", path = "", content = "";
		
		for(String key:keys){
			tmp = key.split("_");
			jsonStr = getRackInfoJsonStr(tmp[0], tmp[1]);
			if("".equals(content)){
				content = jsonStr;
			}else{
				content += "\r\n" + jsonStr;
		...
		path = DataUtils.getMemoryDumpPath(fileName);
		MyFileUtils.saveToFile(content, path);
	}
	
	public void restoreRackInfo(String lineName, String rackID, DeviceState restoreState, boolean informObserver, String fileName){
		String path = "", line = "", rack = "", lastID = "";
		String materialID = "", state = "";
		String[] jsonStrs = null, spec = null;
		...
		try {
			...
			path = DataUtils.getMemoryDumpPath(fileName);
			if(MyFileUtils.getFilePassedDays(path)>0.5) return;
			
			jsonStrs = FileUtils.readFileToString(new File(path), "UTF-8").split("\r\n");
			
			for(String jsonStr:jsonStrs){
				if("".equals(jsonStr)) continue;
				jsonObj = JSONObject.fromObject(jsonStr);
				line = jsonObj.getString("lineName");
				rack = jsonObj.getString("rackID");
				if("All".equals(lineName) || line.equals(lineName)){
					if("All".equals(rackID) || rack.equals(rackID)){
						@SuppressWarnings("unchecked")
						Set<String> set = jsonObj.keySet();
						for(String key:set){
							if(!"lineName".equals(key) && !"rackID".equals(key)
									&& !key.startsWith("state") && !key.startsWith("spec")){
								devState = null;
								materialID = jsonObj.getString(key);
								state = jsonObj.getString("state"+key);
								spec = jsonObj.getString("spec"+key).split(";");
								if(!"".equals(state)) devState = DataUtils.getDevStateByString(state);
								if("".equals(materialID)) continue;
								
								if(putWorkpieceOnRack(line, rack, materialID, key)){
									wpData.setWorkpieceState(materialID, devState);
									if(spec.length > 0) wpData.setData(materialID, WorkpieceItems.PROCESS, spec[0]);
									if(spec.length > 1) wpData.setData(materialID, WorkpieceItems.SURFACE, spec[1]);
									if(spec.length > 2) wpData.setData(materialID, WorkpieceItems.MACHINETIME, spec[2]);
									if(spec.length > 3) wpData.setData(materialID, WorkpieceItems.PROGRAM, spec[3]);
									if(spec.length > 4) wpData.setData(materialID, WorkpieceItems.NCMODEL, spec[4]);
									DataUtils.updateWorkpieceData(materialID, line, "", "");
									if(informObserver && (null == restoreState || devState == restoreState)){
										lastID = materialID;
										scClient.updateMaterialInfo(materialID, SchedulerItems.PORTRACK, true);
									}
								}
							}
						}
			...
			if(!"".equals(lastID)) scClient.updateMaterialInfo(lastID, SchedulerItems.PORTRACK, false);
		} catch (IOException e) {
		}
	}
}

			...
			if(workpieceID.length() > 6){
				...
				if(null != emptySlots){
					...
					if(null == oriID){
						...
						WorkpieceData.getInstance().setData(workpieceID, WorkpieceItems.CONVEYORID, rackID);
						WorkpieceData.getInstance().setData(workpieceID, WorkpieceItems.CONVEYORSLOTNO, emptySlots[0]);
						ok = true;
					}
				}
			}

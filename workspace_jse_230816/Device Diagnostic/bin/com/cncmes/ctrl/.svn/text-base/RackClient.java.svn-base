...
/**
 * For CNC Client to communicate with the Rack Manager
 * @author Zi Long Li
 *
 */
public class RackClient {
	...
	public boolean rackServerIsReady(String lineName, String rackID){
		...
		if(!ready){
			LogUtils.errorLog("RackClient-rackServerIsReady("+lineName+","+rackID+")-sendInfo("+rackCfg[0]+","+rackCfg[1]+","+info+") failed");
		}
		
		return ready;
	}
	
	public boolean updateRackInfo(String lineName, String rackID, String workpieceID, boolean threadMode, boolean threadSequential, boolean theLastThread){
		...
		if(null != dt){
			...
			if(null != spec){
				info += "," + (null!=dt.get(WorkpieceItems.PROCESS)?dt.get(WorkpieceItems.PROCESS):spec.get(SpecItems.PROCESSNAME));
				info += "," + (null!=dt.get(WorkpieceItems.SURFACE)?dt.get(WorkpieceItems.SURFACE):spec.get(SpecItems.SURFACE));
				info += "," + (null!=dt.get(WorkpieceItems.MACHINETIME)?dt.get(WorkpieceItems.MACHINETIME):spec.get(SpecItems.SIMTIME));
				info += "," + (null!=dt.get(WorkpieceItems.NCMODEL)?dt.get(WorkpieceItems.NCMODEL):spec.get(SpecItems.NCMODEL));
				info += "," + (null!=dt.get(WorkpieceItems.PROGRAM)?dt.get(WorkpieceItems.PROGRAM):spec.get(SpecItems.PROGRAM));
			}else{
				...
				info += "," + (null!=dt.get(WorkpieceItems.MACHINETIME)?dt.get(WorkpieceItems.MACHINETIME):"NULL");
		...
		if(!"".equals(info)){
			...
			if(threadMode){
				if(threadSequential){
					ThreadUtils.sequentialRun(new sendMsgToServer(rackCfg[0],Integer.parseInt(rackCfg[1]),info,""), theLastThread);
				}else{
					ThreadUtils.Run(new sendMsgToServer(rackCfg[0],Integer.parseInt(rackCfg[1]),info,""));
				}
			}else{
				success = sendInfo(rackCfg[0],Integer.parseInt(rackCfg[1]),info);
			}
		}
	...
	public String[] getRackEmptySlots(String lineName, String rackID){
		...
		String[] rackCfg = getRackConfig(lineName, rackID);
		if(sendInfo(rackCfg[0],Integer.parseInt(rackCfg[1]),info)){
	...
	class sendMsgToServer implements Runnable{
		...
		@Override
		public void run() {
			boolean OK = sendInfo(ip,port,msg);

...
import com.cncmes.gui.DeviceDiagnostic;
...
/**
 * For CNC Client to communicate with the Scheduler
 * @author Zi Long Li
 *
 */
public class SchedulerClient implements DeviceObserver {
	...
	public boolean updateTask(String taskInfo){
		boolean success = true;
		String info = "updateTask,"+taskInfo;
		info += "," + MathUtils.MD5Encode(info);
	...
	/**
	 * 
	 * @author Zilong Mainly for CNC_Client to receive responses from the Scheduler
	 *
	 */
	...
	/**
	 * 
	 * @author Zilong Mainly for CNC_Client to receive robot info from the Scheduler
	 *
	 */
	...
	/**
	 * 
	 * @author Zilong Mainly for CNC_Client to receive material info from the Scheduler
	 *
	 */
	...
	/**
	 * 
	 * @author Zilong Mainly for Rack_Manager to receive material info from the Scheduler
	 *
	 */
	...
	/**
	 * 
	 * @author Zilong Mainly for CNC_Client to receive task from the Scheduler
	 *
	 */
			...
			if(data.length > 2){
				...
				if(sMD5.equals(MathUtils.MD5Encode(dt))){
					if("getTask".equals(data[0])){
						...
						for(int i=0; i<tasks.length; i++){
							...
							if(null != jsonObj){
								...
								if(null == taskData.getData(curTaskID)){
									if(null == scheduledMachine.get(cncIP) && null == scheduledRobot.get(robotIP)){
										taskData.taskPush(jsonObj.toString());
										scheduledMachine.put(cncIP, cncIP);
										scheduledRobot.put(robotIP, robotIP);
									}
								}
							}
						...
						try {
							String cncIP = (String) TaskData.getInstance().getTaskByID(data[1]).get(TaskItems.MACHINEIP);
							String robotIP = (String) TaskData.getInstance().getTaskByID(data[1]).get(TaskItems.ROBOTIP);
							scheduledMachine.remove(cncIP);
							scheduledRobot.remove(robotIP);
							TaskData.getInstance().removeData(data[1]);
						} catch (Exception e) {
						}
						...
						try {
							...
							DeviceState state = getDevStateByString(jsonObj.getString("taskState"));
							if(null != state){
						...
						if(!"".equals(taskInfo.trim())){
							try {
								...
								for(String task:tasks){
									...
									DeviceState state = getDevStateByString(jsonObj.getString("taskState"));
									if(null != state && (DeviceState.ALARMING == state || DeviceState.HANDLING == state)){
										TaskData.getInstance().taskPush(jsonObj.toString());
									}
								}
							} catch (Exception e) {
								LogUtils.errorLog("getAlarmTask ERR:"+e.getMessage());
							}
							DeviceDiagnostic.getInstance().refreshGUI();
						}
					}
				}
			}
	...
	private DeviceState getDevStateByString(String state){
		...
		if("FIXED".equals(state)) devState = DeviceState.FIXED;
	...
	/**
	 * 
	 * @author Zilong CNC_Client sends message to the Scheduler
	 *
	 */

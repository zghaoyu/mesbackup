...
public class SysConfig extends JDialog {
	...
	/**
	 * MyDataModel class is used to: 1. Show the specific data in a JTable 2.
	 * Specify which data column is editable
	 * 
	 * @author Sanly
	 *
	 */
	...
	private SysConfig() {
		...
		setIconImage(
				Toolkit.getDefaultToolkit().getImage(SysConfig.class.getResource("/com/cncmes/img/setting_24.png")));
		setModal(true);
		setTitle("System Config");
		Dimension d = Toolkit.getDefaultToolkit().getScreenSize();
		int width = 450;
		int height = 450;
		setBounds((d.width - width) / 2, (d.height - height) / 2, 450, 500);
		...
		gbl_contentPanel.rowHeights = new int[] { 30, 187, 30, 130 };
		...
		{
			JLabel lblDatabaseSetting = new JLabel("Mysql Database Setting");
					...
					public void mouseClicked(MouseEvent e) {
						if (1 == e.getButton()) {
							...
							for (int i = 0; i < rowCnt; i++) {
								String paraName = (null != tableCommonSetting.getValueAt(i, 0))
										? String.valueOf(tableCommonSetting.getValueAt(i, 0)).trim() : "";
								String paraVal = (null != tableCommonSetting.getValueAt(i, 1))
										? String.valueOf(tableCommonSetting.getValueAt(i, 1)).trim() : "";
								paraName = paraName.replace("(s)", "").replace("(ms)", "");

								if (!"".equals(paraName) && !"".equals(paraVal)) {
									if ("NULL".equals(paraVal.toUpperCase()))
										paraVal = "";
							...
							for (int i = 0; i < rowCnt; i++) {
								String paraName = (null != tableDatabaseSetting.getValueAt(i, 0))
										? String.valueOf(tableDatabaseSetting.getValueAt(i, 0)).trim() : "";
								String paraVal = (null != tableDatabaseSetting.getValueAt(i, 1))
										? String.valueOf(tableDatabaseSetting.getValueAt(i, 1)).trim() : "";
							...
							if (needToSave) {
								if (!XmlUtils.saveSystemConfig(systemConfig.getDataMap())) {
									JOptionPane.showMessageDialog(contentPanel,
											"Save system config failed\r\nplease check system log for the details",
											"Save System Config Error", JOptionPane.ERROR_MESSAGE);
								} else {
									JOptionPane.showMessageDialog(contentPanel, "Save system config OK",
											"Save System Config OK", JOptionPane.INFORMATION_MESSAGE);
								}
							}
						}
					}

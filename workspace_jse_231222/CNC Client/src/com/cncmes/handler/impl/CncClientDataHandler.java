	...
	public void doHandle(String in, Socket s) {
		...
		if(data.length > 2){
			...
			if(sMD5.equals(MathUtils.MD5Encode(dt))){
				try {
					if(!"schedulerStarted".equals(data[0]) || !"getCncClientState".equals(data[0])){
					...
					switch(data[0]){
					case "getCncClientState":
						String feedback = data[0] + "," + (ThreadController.getStopFlag()?DeviceState.SHUTDOWN:DeviceState.WORKING);
						feedback += "," + MathUtils.MD5Encode(feedback);
						sendMsgToClient(s, feedback);
						break;
					...
					case "schedulerStopped":
						TaskMonitor.disableTaskMonitor();
						break;

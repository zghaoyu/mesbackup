                        ...
                        if (!DebugUtils.isMassProduction()) // need to upload program for batch machining
                        {
                            { //upload program
                                ...
                                try {
                                    ...
                                    Integer filepos = Integer.valueOf(XmlUtils.getCncConfig(cncIP, "workzone" + (index + 1)));
                                ...
                                final String barcode = (String) rtn.get(1);
                                    ...
                                    try {
                                        ...
                                        if (objects != null && objects.size() > 0)               //fixture normal status:1.base on barcode trace processed card 2.base on barcode can get processed program
                                        {
                                            ...
                                            if(materialTrace.getProcessedCardNo() != null && !materialTrace.getProcessedCardNo().equals(""))
                                            {
                                                ...
                                                String cnc_pos = (XmlUtils.getCncConfig(cncIP, "workzone" + rackSlotID));
                                                ...
                                                if (dr != null) {
                                                    ...
                                                    double testTime = (double) infoMap.get("simulate_time");
...
//                                                    System.out.println("at TaskProcessor.getMaterialFromRack" + cncProcessDetail);
...
//                                    return false;
                                    ...
                                    LogUtils.operationLog(logFile, logContPre + "At RACK_" + rackSlotID + " Scan barcode failed or fixture orientation error");
                                }
                            }
                        }
...
//                cncZone = "cnc_72";
            ...
            for (String workZone : workZones) {
                try {
                    spcificZone[num] = Integer.parseInt(XmlUtils.getCncConfig(cncIP, "workzone" + workZone));       //从XML中获取工件在CNC中加工的位置
                    num++;
                } catch (IOException e) {
                    e.printStackTrace();
                } catch (DocumentException e) {
                ...
                if (index < emptyTraySlots.length) {
                    ...
                    if (robotCtrl.placeToName(robotIP, "robot_pallet_a_slot_" + emptyTraySlots[index])) //place to robot
                    {
                        ...
                        if (OrderSchedulers.size() > 0) {
                            ...
                            OrderScheduler orderScheduler = OrderSchedulers.get(index);
...
//					}
    ...
    public boolean loadAndUnloadMaterial(IRobot robotCtrl, String robotIP, CNC cncCtrl, String cncIP,String[] unLoadWorkZones, String[] unloadWorkpieceIDs, String logFile, String logContentPre)
    {
        ...
        LinkedList<String> emptyWorkzonesList = Arrays.stream(emptyWorkzones).collect(Collectors.toCollection(LinkedList::new));
        ...
        LinkedList<String> unloadWorkZoneList = new LinkedList();      //unload WorkZoneSequence:1.unload slot = upload slot 2.others
        LinkedList<String> unloadWorkpieceIDList = new LinkedList();

        LinkedList<String> uploadTraySlotsList = new LinkedList();      //upload RobotTraySlotSequence:1.unload slot = upload slot 2.others
        ...
        for(int i = 0;i<unLoadWorkZones.length;i++ )
        {
            if((uploadPosList.contains(unLoadWorkZones[i])))        //slot position which unload position = load position
            {
                ...
                uploadWorkZoneList.addFirst(unLoadWorkZones[i]);                        //record the cnc
...
//					}
                ...
                case ROBOT_MOVETOMACHINE:
                    ...
                    if (okToMove) {
                        ...
                        if ("10.10.206.72".equals(cncIP)||"10.10.206.73".equals(cncIP)||"10.10.206.74".equals(cncIP)) {
...
//                        if(DeviceState.STANDBY == cncState) {                     //Attention : Uploading program in load/unload action may be caused conflict
                        if(DeviceState.STANDBY == cncState) {
                            for (int i = 1; i <= 14; i++) {
                                String fileName = "O";
                                DecimalFormat decimalFormat = new DecimalFormat("00");
...
//						workpiecesProcessingCircle(wpInfo.get(rackID).split(","));
                ...
                if (cncState == DeviceState.FINISH){
                    ...
                    while (currentState != DeviceState.FINISH)
                    {
                        LogUtils.operationLog(logFile, logContPre + "wait for finishing machine");
...
//                LogUtils.operationLog(logFile, logContPre + "Machine opens door done");
                ...
                LogUtils.operationLog(logFile, logContPre + "CNC door opening");
                ...
                if (cncState == DeviceState.FINISH) {
                    ...
                    Boolean doAction = taskInfo.getTask_info()==null || taskInfo.getTask_info().equals("");
...
//                    }
                    ...
                    if (cncCtrl.checkSensor(cncIP, "3304", "1"))     //detect whether the door is opened
                    {
                        ...
                        if(!doAction)
                        {
                            ...
                            cncData.setData(cncIP, CncItems.FINISHSHOW, "");
                            cncData.clearWorkpieceID(cncIP);//Clear the workpieces in CNC
                            LogUtils.operationLog(logFile, logContPre + "Machine prepare to finish,wait machine standBy then load/unload together,CNC door open fail");
...
//					if(!startMachining(cncCtrl,cncIP,mainProgram,3,10)){
                    if (!cncCtrl.startMachining(cncIP, "")) {     //close door and machining
                        ...
                        for (int i = 0; i < 15; i++)        //15s detect whether the door is closed
                        {
...
//                    cncCtrl.startMachining(cncIP,"");
                    ...
                    if (oppenDoor)        //detect whether the door is opened
                    {
                        ...
                        TaskInfo task = new TaskInfo(taskID,jsonObject.toString(),cncIP,0,simulateTime);
...
//                    }
                    ...
                    if (runConcurrentOP(DeviceOP.ROBOT_GOTOCHARGE, cncIP, robotIP, json, cncCtrl, robotCtrl, 1, 60)) {//change 30 second to 60 by Hui Zhi  2022/5/18

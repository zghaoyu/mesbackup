...
public class XmlUtils {
    ...
    public static void parseSystemConfig() {
        ...
        sqlServerCfg.put("url","jdbc:sqlserver://10.10.204.82:1433; DatabaseName=UFDATA_001_2018; encrypt=false");
        sqlServerCfg.put("username", "u8readonly");
        sqlServerCfg.put("userpwd", "u8readonly");
        sqlServerCfg.put("driver", "com.microsoft.sqlserver.jdbc.SQLServerDriver");
    ...
    public static String getCncConfig(String ip, String propertyName) throws IOException, DocumentException {
        String path = getXmlFilePath(true,"CncConfig.xml");
//1、获取输入流
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (element.elementText("ip").equals(ip)) {
                return element.elementText(propertyName);
            }
        }
        fis.close();
    ...
    public static void setCncConfig(String ip, String propertyName, String value) {
        try {
            String path = getXmlFilePath(true,"CncConfig.xml");
            FileInputStream fis = new FileInputStream(path);
            SAXReader sr = new SAXReader();
            Document doc = sr.read(fis);
            Element root = doc.getRootElement();
            Boolean haveCnc = false;
            List<Element> list = root.elements();
            for (int i = 0; i < list.size(); i++) {
                Element element = list.get(i);
                if (element.elementText("ip").equals(ip)) {
                    Element property = element.element(propertyName);
                    if (property != null) {
                        property.setText(value);
                    }
                    else {
                        Element e = element.addElement(propertyName);
                        e.setText(value);
                    }
                    haveCnc = true;
                }
            }
            if (!haveCnc)    // 该ip的CNC 尚未配置
            {
                Element cnc = root.addElement("CNC");
                Element cncip = cnc.addElement("ip");
                cncip.setText(ip);
                Element element = cnc.addElement(propertyName);
                element.setText(value);
            }
            saveDocument(doc, new File(path));
        } catch (FileNotFoundException | DocumentException e) {
            e.printStackTrace();
            LogUtils.errorLog("XmlUtils-saveCncConfigXml ERR:" + e.getMessage());
        } catch (IOException e) {
            e.printStackTrace();
        }

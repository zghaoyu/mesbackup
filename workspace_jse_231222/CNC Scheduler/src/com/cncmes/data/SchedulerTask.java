...
public class SchedulerTask extends RunningData<String> {
		...
		if(null != dt){
			try {
				...
				for(String taskID:dt.keySet()){
					...
					if("STANDBY".equals(taskState) || "PREPAREFINISH".equals(taskState)){
						if("".equals(info)){
	...
	public void addTaskInfo(String lineName, String taskID, String taskInfo){
		...
		CNCTask cncTask = new CNCTask();
		cncTask.setTask_info(taskInfo);
		cncTask.setTask_id(taskID);
		cncTask.setLine_name(lineName);
		cncTask.setCreate_time(TimeUtils.getCurrentDate("yyyy-MM-dd HH:mm:ss"));
		DataUtils.saveTask(cncTask);
		//end

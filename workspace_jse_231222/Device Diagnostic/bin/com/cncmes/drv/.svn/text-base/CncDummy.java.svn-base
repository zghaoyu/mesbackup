...
public class CncDummy implements CNC {
	...
	public boolean sendCommand(String ip, String operationName, LinkedHashMap<String, String> inParas, LinkedHashMap<String, String> outParas){
		...
		if(null == sc.getSocket(ip, port) || null == socketRespData.get(dtKey)){
			try {
				cnnOK = sc.connect(ip, port, sdh, DummyItems.IP_CNC, cmdEndChr);
		...
		for(String cmd:cmds){
			...
			for(int i=0; i<=cmdRetryCount; i++){
				if(i > 0){
					pingInfo = NetUtils.pingHost(DebugUtils.getDummyDeviceIP(DummyItems.IP_CNC));
					if(!"OK".equals(pingInfo)){
						LogUtils.commandLog(ip, "NG"+LogUtils.separator+operationName+LogUtils.separator+cmd+LogUtils.separator+"Ping("+DebugUtils.getDummyDeviceIP(DummyItems.IP_CNC)+") Failed:"+pingInfo);
						break;
					}else{
						LogUtils.commandLog(ip, "NG"+LogUtils.separator+operationName+LogUtils.separator+cmd+LogUtils.separator+"Ping("+DebugUtils.getDummyDeviceIP(DummyItems.IP_CNC)+") OK");
					}
				}
				if(i == 2) cnnOK = false;
	...
	class CncSocketDataHandler extends SocketDataHandler{
		...
		public void doHandle(String in, Socket s) {
			...
			if("127.0.0.1".equals(ip) || DebugUtils.getDummyDeviceIP(DummyItems.IP_CNC).equals(ip)) ip = portMap.get(""+s.getPort());

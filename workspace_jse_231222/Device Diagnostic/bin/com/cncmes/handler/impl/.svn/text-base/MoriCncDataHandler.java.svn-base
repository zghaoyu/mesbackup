...
public class MoriCncDataHandler implements CncDataHandler {
	private static CncWebAPI cncWebAPI = CncWebAPI.getInstance();
	private static MoriCncDataHandler dataHandler = new MoriCncDataHandler();
	private MoriCncDataHandler(){}
	
	public static MoriCncDataHandler getInstance(){
		return dataHandler;
	}
	...
	public LinkedHashMap<String, String> alarmInfoHandler(String alarmInfo) {
		...
		if(data.length > 0){
			for(int i=0; i<data.length; i++){
				...
				info.put(dt.getString("code"), dt.getString("msg"));
			}
		}
		return info;
	}
	...
	public String machineMainProgramNameHandler(String machiningPrograms) {
		...
		try {
			...
			LogUtils.errorLog("MoriCncDataHandler machineMainProgramNameHandler Err:"+e.getMessage()+LogUtils.separator+machiningPrograms);
		}
	...
	public String machineCurrSubProgramNameHandler(String machiningPrograms) {
		...
		try {
			...
			LogUtils.errorLog("MoriCncDataHandler machineCurrSubProgramNameHandler Err:"+e.getMessage()+LogUtils.separator+machiningPrograms);
		}

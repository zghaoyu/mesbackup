 ...
 * 
 * @author W000650
 * Access the database via Data Transfer Object
 *
 */
public class DAOImpl implements DAO {
	...
	/**
	 * 
	 * @param dtoClassName class name of the Data Transfer Object
	 * access the database via Data Transfer Object
	 */
		...
		while(it.hasNext()){
			...
			if(!"id".equals(fname)){
				...
				try {
					...
					f.setAccessible(true);
					id = f.get(dto) instanceof Integer?(int)f.get(dto):(long)f.get(dto);
				} catch (Exception e) {
					throw new SQLException("Reflect DTO Declared Field fail");
				}
			}
	...
	public ArrayList<Object> findJOInfoByMoCode(String mocode) throws SQLException {
		String sql = "select mo.MoId,mo.MoCode,inv.cInvCode as PartNumber,inv.cInvName as DWG,mos.Qty,sfc_md.OpSeq,sfc_o.OpCode\n" +
				"from mom_order mo\n" +
				"left join mom_orderdetail mos on mo.MoId=mos.MoId\n" +
				"left join Inventory inv on inv.cInvCode=mos.InvCode\n" +
				"left join sfc_moroutingdetail sfc_md on sfc_md.MoDId=mos.MoDId\n" +
				"left join sfc_operation sfc_o on sfc_o.OperationId=sfc_md.OperationId\n" +
				"where mo.MoCode='" + mocode+
				"' order by sfc_md.OpSeq asc";
					...
					try {
						...
						while(it.hasNext()){
							...
							if(!"id".equals(fname)){
								...
								if(type.equals(BigDecimal.class)) f.set(vo, rs.getBigDecimal(idx));

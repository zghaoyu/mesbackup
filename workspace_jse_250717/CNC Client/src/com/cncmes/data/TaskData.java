...
public class TaskData extends RunningData<TaskItems> implements DeviceSubject {
	 ...
	 * @return true when the task is pushed successfully, the task is only pushed when the machine state is STANDBY or FINISH or PREPAREFINISH
	 */
	public synchronized boolean taskPush(String taskInfo){
		...
		if(cncState == DeviceState.FINISH || cncState == DeviceState.STANDBY || cncState == DeviceState.PREPAREFINISH){
			...
			LogUtils.operationLog(TimeUtils.getCurrentDate("yyyyMMdd")+"_"+cncIP+"_TaskData.log","Task info  :  "+taskInfo);

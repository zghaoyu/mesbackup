...
//		try {
//			List<Object> dr = processDetailDao.getCNCProcessDetail("10.10.206.178");
...
//		}
		...
		chckbxHMLV = new JCheckBox("HMLV");
		chckbxHMLV.setSelected(true);
		chckbxHMLV.addItemListener(new ItemListener() {
			@Override
			public void itemStateChanged(ItemEvent e) {
				DebugUtils.setHMLV(chckbxHMLV.isSelected());
				System.out.println(DebugUtils.isHMLV());

			}
		});
		chckbxHMLV.setFont(new Font("Microsoft YAHEI", Font.PLAIN, 12));
		mnDebug.add(chckbxHMLV);
		...
		btnReset.setIcon(new ImageIcon(CNCClient.class.getResource("/com/cncmes/img/reset.png")));
		toolBar_commonOp.add(btnReset);
...
//			if(null!=cncClient) cncClient.setVisible(true);
	...
	{
		DAOImpl processDetailDao = new DAOImpl("com.cncmes.dto.CncProcessDetail");
		DAOImpl taskInfoDao = new DAOImpl("com.cncmes.dto.TaskInfo");
		try {
			List<Object> taskInfo = taskInfoDao.findByCnd(new String[]{"have_done"}, new String[]{"0"});
			List<Object> processDetails = processDetailDao.monitorDataBaseFindByCnd(new String[]{"have_finish"},new String[]{"0"});
			if(taskInfo != null && taskInfo.size() != 0)
			{
				for(Object t : taskInfo)
				{
					TaskInfo task = (TaskInfo) t;
					task.setHave_done(1);
					if(taskInfoDao.update(task) == 0)
					{
					 	System.out.println("Table task_info reset fail");
						JOptionPane.showMessageDialog(null,"Table task_info reset fail");
					}
				}
			}
			if(processDetails != null && processDetails.size() != 0)
			{
				for(Object p : processDetails)
				{
					CncProcessDetail cncProcessDetail = (CncProcessDetail) p;
					cncProcessDetail.setHave_finish(1);
					if(processDetailDao.monitorDBupdate(cncProcessDetail) == 0)
					{
						System.out.println("Table cnc_process_detail reset fail");
						JOptionPane.showMessageDialog(null,"Table cnc_process_detail reset fail");
					}
				}
			}
			...
			JOptionPane.showMessageDialog(null,"Reset fail");
			throwables.printStackTrace();
		}

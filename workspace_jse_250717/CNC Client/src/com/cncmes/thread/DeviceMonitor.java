...
import com.cncmes.dto.TaskInfo;
...
public class DeviceMonitor {
	...
	public void cncMonitoring(String cncIP){
		...
		if(DeviceState.SHUTDOWN == devState){
			...
			if(DeviceState.SHUTDOWN == devState) return;	//CNC has been shutdown
...
//						"<p style=\"text-indent: 3em;\">I so sorry to inform you of MES run fail.Please check the attachment for details.CNC status is alarming</p>" +
//						"<p>Best Regards.</p>" ;
//				try {
...
//			}
		...
		if(DeviceState.WORKING == devState && DeviceState.WORKING == lastState){
			...
			if(DeviceState.WORKING == realtimeState){
				...
				//check remainder machining time
				DAO dao = new DAOImpl("com.cncmes.dto.TaskInfo");
				TaskInfo taskInfo = new TaskInfo();
				List<Object> os = null;
				try {
					os = dao.findByCnd(new String[]{"cnc_ip", "have_done"}, new String[]{cncIP, "0"});
					if(os != null) {
						for (Object o : os) {
							taskInfo = (TaskInfo) o;
						}
						...
						if (prepareNextTaskFlag) {
							System.out.println("change status to PREPAREFINISH : "+currentDate);
							System.out.println("begin process time : "+beginDate);
							System.out.println("simulate time : "+taskInfo.getSimulate_time());
							cncData.setCncState(cncIP, DeviceState.PREPAREFINISH, true, false, true, false);
						}
					}
...
//				String recipients = "HY_Zhong@sto-tech.hk;jianheng_xiao@sto-tech.hk";
//				ErrorEMailUtil eMailUtil = new ErrorEMailUtil("mail1.sto-tech.hk","465","Eddie","HY_Zhong@sto-tech.hk","W000650STO","HY_Zhong@sto-tech.hk",recipients);
//				String content = "<p>Hello.</p>" +
...
//				//---------------------------------------------------------------------------------
		...
		if(DeviceState.PREPAREFINISH == devState)
		{
			lineName = cncData.getLineName(cncIP);
			if(!"".equals(taskID)) robotIP = (String) taskData.getData(taskID).get(TaskItems.ROBOTIP);
			logFile = LogUtils.getOperationLogFileName(cncIP, cncModel);
			...
			LogUtils.operationLog(logFile, logContPre+"Machining prepare to finish");
			for(String workpieceID:wpIDs.values()){
				procNo = wpData.getCurrentProcNo(workpieceID);
				wpData.setProcessState(workpieceID, procNo, DeviceState.FINISH);
			}
			if(!"".equals(taskID)) taskData.taskPopByID(taskID, true); //Finish machining
			cncData.setCncState(cncIP, DeviceState.PREPAREFINISH, true, false, true, false);
			cncData.setCncLastState(cncIP, DeviceState.PREPAREFINISH);
			cncData.setFinishMachiningTime(cncIP, System.currentTimeMillis());
			workZones.remove(cncIP);
			LogUtils.operationLog(logFile, logContPre+"Unlock machine done");
		}

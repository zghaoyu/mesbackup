						...
						if(!"".equals(wpIDs)){
							...
							for(int i=0; i<ids.length; i++){
								if(rackM.putWorkpieceOnRack(materialCurLine, racks[i], ids[i], slots[i])){
									...
									if((int)wpData.getItemVal(ids[i], WorkpieceItems.PROCQTY)>=0){	//disable this type of upload program     if((int)wpData.getItemVal(ids[i], WorkpieceItems.PROCQTY)>0)
...
//								workpieceID = mesCodes[(((i+1) % mesCnt)>0?((i+1) % mesCnt):mesCnt)-1] + System.currentTimeMillis() + (new Random().nextInt(100));
								workpieceID = mesCodes[(i%5)] + System.currentTimeMillis() + (new Random().nextInt(100));    //only need SCS drawing in order to test comfortably
	...
	/**
	 * 
	 * @param guiType 0:RackMaterial 1:RackProduct
	 */
	...
	private boolean machiningProgramsReady(String workpieceID, int procQty){	//disable this type of upload program

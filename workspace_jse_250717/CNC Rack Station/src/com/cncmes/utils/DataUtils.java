...
public class DataUtils {
	...
	public static Fixture getFixtureById(int fixture_id) {
		DAO dao = new DAOImpl(dtoFixture);
		Fixture vos = null;

		try {
				vos = (Fixture) dao.findById(fixture_id);
				if (null != vos) {
					return vos;
				}
	...
	public static boolean saveFixtureMaterial(int fixtureId, int materialId,int cncLineId,int rackId) {
		...
		dto.setLine_id(cncLineId);
		dto.setRack_id(rackId);
	...
	public static ArrayList<FixtureMaterial> getFixtureMaterialOfRack(int fixtureId, int is_released, int is_deleted,int line_id,int rack_id) {
		ArrayList<Object> vos = null;
		ArrayList<FixtureMaterial> result = new ArrayList<>();
		FixtureMaterial vo;
		DAO dao = new DAOImpl(dtoFixtureMaterial);
		...
		try {
			vos = dao.findByCnd(new String[] { "fixture_id", "is_released", "is_deleted","line_id","rack_id" },
					new String[] { "" + fixtureId, "" + is_released, "" + is_deleted, ""+line_id, ""+rack_id });
			if (null != vos) {
				result = new ArrayList<FixtureMaterial>();
				for (int i = 0; i < vos.size(); i++) {
					vo = (FixtureMaterial) vos.get(i);
	...
	public static ArrayList<FixtureMaterial> getFixtureMaterialByMaterialId(int materialId, int is_released, int is_deleted,int line_id,int rack_id)
	{
		ArrayList<Object> vos = null;
		ArrayList<FixtureMaterial> result = new ArrayList<>();
		FixtureMaterial vo;
		DAO dao = new DAOImpl(dtoFixtureMaterial);
		try {
			vos = dao.findByCnd(new String[] { "material_id", "is_released", "is_deleted","line_id","rack_id" },
					new String[] { "" + materialId, "" + is_released, "" + is_deleted, ""+line_id, ""+rack_id });
			if (null != vos) {
				result = new ArrayList<FixtureMaterial>();
				for (int i = 0; i < vos.size(); i++) {
					vo = (FixtureMaterial) vos.get(i);

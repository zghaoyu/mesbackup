...
import com.cncmes.utils.MonitorDBUtils;
...
public class SchedulerServer {
    ...
    private SocketServer workingCircle = null;
    private Boolean getNextTask = false;
    ...
    private static int RACK_ACTUAL_CAPACITY = 14;
...
//	private static LinkedHashMap<ClientType,LinkedHashMap<String,String>> myClients = new LinkedHashMap<ClientType,LinkedHashMap<String,String>>();
    ...
    public String start() {
        ...
        workingCircle = SocketServer.getInstance();
        try {
            workingCircle.socketSvrStart(8765, new WorkPieceWokingCircleHandler());
        } catch (IOException e) {
            e.printStackTrace();
            return "COM port(8765) for working circle starts failed\r\n" + e.getMessage();
        }
        machineSS = SocketServer.getInstance();
        try {
            machineSS.socketSvrStart(PORTMACHINE, new MachineMsgHandler());
        } catch (IOException e) {
            workingCircle.stopSvrPort(8765);
        ...
        RackMaterialMonitor.getInstance().run("");//TODO add by Hui Zhi 2022/7/8  for testing Ӧ����һ�����ܲ��ϼ��
    ...
    public String stop() {
        ...
        if (null != workingCircle) workingCircle.stopSvrPort(8765);
        if (schedulerThread.size() > 0) {
            for (String lineName : schedulerThread.keySet()) {
    ...
    //TODO 2022/7/13 Hui Zhi
    private void LoadMaterialData() {
        //Find all fixture on rack storage
    ...
    class MachineMsgHandler implements SocketRespHandler {
        ...
        public void doHandle(String in, Socket s) {
            ...
            System.out.println("at SchedulerServer.MachineMsgHandler:" + Arrays.deepToString(data));
            if (data.length > 2) {
                ...
                if (sMD5.equals(MathUtils.MD5Encode(dt))) {
                    ...
                    if ("setState".equals(data[0])) {
                        ...
                        if (null == oriState || null != oriState && (DeviceState) oriState != curState) {
                            ...
                            if (DeviceState.STANDBY == curState || DeviceState.PREPAREFINISH ==curState) removeScheduledMachine(data[1]);
                        }
                        ...
                        {
                            sMachine.setData(data[0], SchedulerDataItems.STATE, DeviceState.FINISH);
                        }

                        sMachine.setData(data[0], SchedulerDataItems.MODEL, data[4]);
                        sMachine.setData(data[0], SchedulerDataItems.MACHINETIME, data[6]);
                        sMachine.setData(data[0], SchedulerDataItems.WKZONEQTY, data[7]);   //data[7]=6
                        ...
                        sMachine.setData(data[0], SchedulerDataItems.SLOT7, data[15]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT8, data[16]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT9, data[17]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT10, data[18]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT11, data[19]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT12, data[20]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT13, data[21]);
                        sMachine.setData(data[0], SchedulerDataItems.SLOT14, data[22]);
                        sMachine.setData(data[0], SchedulerDataItems.WPIDS, data[23]);

                        sMachine.setData(data[0], SchedulerDataItems.WPSLOTIDS, data[24]);
                        sMachine.setData(data[0], SchedulerDataItems.WPSTATES, data[25]);
                        sMachine.setData(data[0], SchedulerDataItems.CONTROLCENTER, data[26] + ":" + data[27]);
                        bMachineDataSyn.put(lineName, false);
                        devControlCenter.put(lineName, data[26] + ":" + data[27] + ":OK"); //IP:Port
//                        System.out.println("    ---------------------startScheduler : " + lineName);
    ...
    // make schedule like a queue and the system cycle run
    class WorkPieceWokingCircleHandler implements SocketRespHandler {
        @Override
        public void doHandle(String in, Socket s) {
            try {
                Thread.sleep(200);
            ...
            if (null == s || null == in) return;
            String[] data = in.split(";");
            System.out.println("WorkPieceWokingCircleHandler : " + Arrays.deepToString(data));
            if (!in.equals("Client Server started")) {
                SchedulerMaterial material = SchedulerMaterial.getInstance();
                material.setSpecifiedMaterialStatus(data, DeviceState.STANDBY);
            }
        }
    }
    ...
    class RobotMsgHandler implements SocketRespHandler {
        ...
        public void doHandle(String in, Socket s) {
            ...
            String[] data = in.split(",");
//			System.out.println("at RobotMsgHandler:"+Arrays.deepToString(data));
            if (data.length > 2) {
                String sMD5 = data[data.length - 1];
                String dt = in.substring(0, in.length() - sMD5.length() - 1);
                if (sMD5.equals(MathUtils.MD5Encode(dt))) {
                    ...
                    if ("setState".equals(data[0])) {
                        ...
                        sRobot.setData(data[0], SchedulerDataItems.CAPACITY, data[7]);   //robot上的slot
...
//						System.out.println("at SchedulerServer.MaterialMsgHandler receive msg and set setMaterialData:"+ Arrays.deepToString(data));
                        schedulerGUI.refreshGUI(2);
                        schedulerGUI.refreshGUI(3);
            ...
            if (data.length > 2) {
                ...
                if (sMD5.equals(MathUtils.MD5Encode(dt))) {
                    ...
                    if ("setState".equals(data[0])) {
                        ...
                        RunningMsg.set(in);
                        sendMsgToClient(s, in);
                        System.out.println("at SchedulerServer.RackMsgHandler receive msg and set MaterialData:" + Arrays.deepToString(data));
                        setMaterialData(data, in);
                        schedulerGUI.refreshGUI(2);
                        schedulerGUI.refreshGUI(5);
                    }
    ...
    /**
     * @author Zilong
     * Mainly for Scheduler to receive requests from CNC_Client and Device Diagnostic Client
     */
    class TaskMsgHandler implements SocketRespHandler {
        ...
        public void doHandle(String in, Socket s) {
            if (null == s || null == in) return;
            String[] data = in.split(",");
//			System.out.println("at SchedulerServer.TaskMsgHandler:"+Arrays.deepToString(data));
            if (data.length > 2) {
                String sMD5 = data[data.length - 1];
                String dt = in.substring(0, in.length() - sMD5.length() - 1);
                if (sMD5.equals(MathUtils.MD5Encode(dt))) {
                    ...
                    if ("checkReady".equals(data[0])) {
                        ...
                        sendMsgToClient(s, in);
                        String ccInfo = devControlCenter.get(data[1]);//ccInfo is "IP:PORT:OK" or"IP:PORT:NG".  Comment by Hui Zhi 2022/7/1
                        System.out.println("pauseSchedulerForLine  :"+ccInfo);
                        ...
                        System.out.println("resumeSchedulerForLine : "+ccInfo);
                        if (null != ccInfo && ccInfo.indexOf(":NG") >= 0) {
                            ccInfo = ccInfo.replace(":NG", ":OK");
                            devControlCenter.put(data[1], ccInfo);
                        }
                    } else if ("getTask".equals(data[0])) {//data[0] is command, data[1] is lineName. Comment by Hui Zhi 2022/7/1
                        String taskInfo = schedulerTask.getTaskInfoByLineName(data[1]);
                        System.out.println("getTask.taskInfo  : "+taskInfo);
                        if (!"".equals(taskInfo)) {
                    ...
                    }else if("getNextTask".equals(data[0])){
                        String lineName = data[1];
                        String taskID = data[2];
                        getNextTask = true;

                    }
                    else if ("removeTask".equals(data[0])) {//data[0]:command | [1]:taskID | [2]:linename. Comment by Hui Zhi 2022/7/1
                        String taskInfo = (String) schedulerTask.getData(data[2]).get(data[1]);
                        System.out.println("removeTask  :"+taskInfo);
                        if (null != taskInfo) {
                        ...
                        System.out.println("updateTask  :"+taskInfo);
                        try {
                            JSONObject jsonObj = JSONObject.fromObject(taskInfo);
                            ...
                            if (DeviceState.ALARMING == taskState) {
                                ...
                                if (DeviceState.STANDBY == machineState || DeviceState.FINISH == machineState)
                        ...
                        if (!"".equals(taskInfo)) {
                            String cmd = data[0] + "," + taskInfo;
                            cmd += "," + MathUtils.MD5Encode(cmd);
                            System.out.println("getAlarmTask  :"+cmd);
                            sendMsgToClient(s, cmd);
                        } else {
                            sendMsgToClient(s, in);
                        }
                        ...
                        String cmd = data[0] + "," + taskInfo;
                        cmd += "," + MathUtils.MD5Encode(cmd);
                        System.out.println("getTaskInfo  :"+cmd);
                        sendMsgToClient(s, cmd);
                    }
                }
            }
        ...
        public void run() {
            ...
            while (!ThreadController.getSchedulerStopFlag()) {
                try {
                    ...
                    if (null != bMachineDataSyn.get(lineName)) machineDataSyn = bMachineDataSyn.get(lineName);        //check whether have info input SchedulerServer
                    ...
                    if (machineDataSyn || robotDataSyn || materialDataSyn) {        //machine,robot and material socket listener need to receive new info
                    ...
                    System.out.println("begin-----------------------------------------------------" + materialList.size());
                    for (Map.Entry<String, LinkedHashMap<SchedulerDataItems, Object>> entry : materialList.entrySet()) {
                        System.out.println("at SchedulerExecution.SchedulerExecution.materialList:" + "Key = " + entry.getKey() + ", Value = " + entry.getValue());
                    }
                    System.out.println("end-------------------------------------------------------");


                    //getNextTask is prepare for load/unload in one time
                    if ((machineList.size() > 0 && robotList.size() > 0)) {
                        ...
                        for (String rKey : robotList.keySet()) {
                            ...
                            if (DeviceState.STANDBY == robotState && !robotIsScheduled(robotIP)) break;         //robot standby and idle
                        }
                        if (DeviceState.STANDBY != robotState || robotIsScheduled(robotIP)) {               //not idle
                        ...
                        //First priority : Server for the STANDBY machine
...
//                                if (machineIsScheduled(machineIP)) continue;//Machine has been scheduled
                                if(machineState == DeviceState.PREPAREFINISH) removeScheduledMachine(machineIP);

                                if (machineIsScheduled(machineIP) && machineState != DeviceState.PREPAREFINISH) continue;//Machine has been scheduled

//                                if (DeviceState.STANDBY == machineState || (DeviceState.WORKING == machineState && getNextTask)) {
                                if (DeviceState.STANDBY == machineState || DeviceState.PREPAREFINISH == machineState) {
                                    System.out.println("at SchedulerExecution machine : "+machineIP+"  machineState : "+machineState.name());

                                    if(DeviceState.STANDBY == machineState)
                                    {
                                        emptyZones = machine.getEmptyWorkzones(machineKey);
//									System.out.println("at SchedulerExecution machine emptyZones="+Arrays.deepToString(emptyZones));
...
//									System.out.println("at SchedulerExecution machine empty workzoneQTY length="+workzoneQTY+"     machine zonesQty="+zonesQty+"      trayEmptySlots length="+trayEmptySlots.length);
                                        ...
                                        zonesQty = machine.getWorkzonesQty(machineKey);
                                        qty = trayEmptySlots.length > zonesQty ? zonesQty:trayEmptySlots.length;
                                    }

//									System.out.println("at SchedulerExecution qty:"+qty);
                                    cncModel = (String) machine.getData(machineKey).get(SchedulerDataItems.MODEL);
//									System.out.println("cncModel:"+cncModel);
                                    wpIDs = "";
                                    rackIDs = "";
                                    slotIDs = "";
                                    wpStates = "";
                                    ...
                                    for (String workpieceID : materialList.keySet()) {
                                        if(workpieceID.startsWith("CNC"))                   //material
                                        {
                                            String material_cncip;
                                            material_cncip = getCNCipByName(workpieceID.substring(0,6));    //CNC079
                                            if(material_cncip != "" && material_cncip.equals(machineIP))
                                            {
                                                continue;
                                            }
                                        }
                                        ...
                                        //make logical slot ID exchange to actual slot
                                        if((Integer.valueOf(slotID) > RACK_ACTUAL_CAPACITY))
                                        {
                                            int actualSlot = Integer.valueOf(slotID) % (RACK_ACTUAL_CAPACITY);  //18 % 14 = 4
                                            slotID = String.valueOf(actualSlot);
                                        }


                                        if (ncModels.indexOf(cncModel) >= 0) {        //select the Workpiece that can be machined by the CNC
                                            i++;
                                            if (i >= qty) break;
                                            material.setMaterialState(workpieceID, DeviceState.PLAN);          //更改workpiece状态位plan
                                            if ("".equals(wpIDs)) {
                                                wpIDs = workpieceID;
                                                rackIDs = rackID;
                                                slotIDs = slotID;
...
//                        //                    if materials all are processed ,change cnc status from PREPAREFINISH to FINISH
//                        if(materialList.size() <= 0 && machineState == DeviceState.PREPAREFINISH)
//                        {
//                            machineState = DeviceState.FINISH;
//                        }
        ...
        public String getCNCipByName(String cncName){
            List<Map> result = MonitorDBUtils.execute("select cnc_ip from cnc where name = '"+cncName +"'");
            String name = (String) result.get(0).get("cnc_ip");

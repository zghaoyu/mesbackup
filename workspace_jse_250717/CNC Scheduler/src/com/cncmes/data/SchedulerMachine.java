		...
		if(dataMap.size() > 0){
			for(String key:dataMap.keySet()){
				...
				if(lineName.equals(ln)){
					...
					if(DeviceState.STANDBY == devState || DeviceState.FINISH == devState || DeviceState.PREPAREFINISH == devState) dt.put(key, data);
				}
			}
		}
	}
	...
	public String[] getEmptyWorkzones(String key){
		...
		if(null != dt){
			...
			for(int i=1; i<=workzoneQty; i++){
				if(1 == i) val = dt.get(SchedulerDataItems.SLOT1);
				if(2 == i) val = dt.get(SchedulerDataItems.SLOT2);
				if(3 == i) val = dt.get(SchedulerDataItems.SLOT3);
				if(4 == i) val = dt.get(SchedulerDataItems.SLOT4);
				if(5 == i) val = dt.get(SchedulerDataItems.SLOT5);
				if(6 == i) val = dt.get(SchedulerDataItems.SLOT6);
				if(7 == i) val = dt.get(SchedulerDataItems.SLOT7);
				if(8 == i) val = dt.get(SchedulerDataItems.SLOT8);
				if(9 == i) val = dt.get(SchedulerDataItems.SLOT9);
				if(10 == i) val = dt.get(SchedulerDataItems.SLOT10);
				if(11 == i) val = dt.get(SchedulerDataItems.SLOT11);
				if(12 == i) val = dt.get(SchedulerDataItems.SLOT12);
				if(13 == i) val = dt.get(SchedulerDataItems.SLOT13);
				if(14 == i) val = dt.get(SchedulerDataItems.SLOT14);

...
public class SchedulerRobot extends RunningData<SchedulerDataItems> {
	...
	private String getRobotInfoJsonStr(String key){
		...
		if(null != data){
			...
			jsonObj.put("slot10", ""+data.get(SchedulerDataItems.SLOT10));
			jsonObj.put("slot11", ""+data.get(SchedulerDataItems.SLOT11));
			jsonObj.put("slot12", ""+data.get(SchedulerDataItems.SLOT12));
			jsonObj.put("slot13", ""+data.get(SchedulerDataItems.SLOT13));
			jsonObj.put("slot14", ""+data.get(SchedulerDataItems.SLOT14));
			...
			if(MyFileUtils.getFilePassedDays(path)<=0.5){
				try {
					...
					if(null == restoreState || devState == restoreState){
						...
						setData(key, SchedulerDataItems.SLOT10, ""+jsonObj.getString("slot10"));
						setData(key, SchedulerDataItems.SLOT11, ""+jsonObj.getString("slot11"));
						setData(key, SchedulerDataItems.SLOT12, ""+jsonObj.getString("slot12"));
						setData(key, SchedulerDataItems.SLOT13, ""+jsonObj.getString("slot13"));
						setData(key, SchedulerDataItems.SLOT14, ""+jsonObj.getString("slot14"));
	...
	public String[] getEmptySlots(String key){
		...
		if(capacity > 0){
			for(int i=1; i<=capacity; i++){
				...
				if((null != itemVal && "".equals(itemVal))|| null == itemVal){
					if("".equals(temp)){
						temp = "" + i;
					}else{
						temp += "," + i;
	...
	public String getSlotVal(String key, int slotNo){
		...
		if(10 == slotNo) itemVal = getItemVal(key, SchedulerDataItems.SLOT10);
		if(11 == slotNo) itemVal = getItemVal(key, SchedulerDataItems.SLOT11);
		if(12 == slotNo) itemVal = getItemVal(key, SchedulerDataItems.SLOT12);
		if(13 == slotNo) itemVal = getItemVal(key, SchedulerDataItems.SLOT13);
		if(14 == slotNo) itemVal = getItemVal(key, SchedulerDataItems.SLOT14);

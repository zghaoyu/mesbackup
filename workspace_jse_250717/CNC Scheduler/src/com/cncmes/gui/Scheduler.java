...
public class Scheduler extends JFrame {
	 ...
	 */
	private Scheduler() {
		saveFirstTimeOrderInfo(getAllOrderSchedulers());
...
//add at 2023 11 28-----------------------------------------------------------------------------------------
		...
		lblCurrentProcessMoCode = new JLabel("  ");
	...
	private List<OrderScheduler> getAllOrderSchedulers()
	{
		...
		for (CNCProcessCard cncProcessCard : cncProcessCards)
		{
			...
			try {
				...
				e.printStackTrace();
				LogUtils.errorLog("Get processed time error at NC File " + orderInfo.getMoCode());
				orderInfo.setProcesstime(0);
			}
...
//		System.out.println(orderSchedulers);
	...
	private Boolean saveFirstTimeOrderInfo(List<OrderScheduler> orderSchedulers)        //record order remainder of the need to machining materials.
	{
		Boolean success = true;
		for(OrderScheduler order : orderSchedulers)
		{
			DAOImpl dao = new DAOImpl(orderSchedulerDto);
			try {
				List<Object> os = dao.findByCnd(new String[]{"moCode"},new String[]{order.getMoCode()});
				if(os == null)
				{
					order.setHave_finish(0);
					dao.add(order);
				}

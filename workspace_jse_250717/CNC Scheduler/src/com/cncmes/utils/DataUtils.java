...
public class DataUtils {
	...
	public static void getMachiningSpec(){
		MachiningSpec mSpec = MachiningSpec.getInstance();
	...
	public static void getDeviceDriver(){
	...
	public static void getRacksInfo(){
	...
	public static void getSchedulerConfig(){
	...
	//Add by Hui Zhi  2022/7/5
	public static boolean saveTask(CNCTask task){
		boolean success = true;
		DAO dao = new DAOImpl("com.cncmes.dto.CNCTask");
		try {
			dao.add(task);
		} catch (SQLException e) {
			success = false;
	...
	// Add by Hui Zhi 2022/7/7 Testing
	public static ArrayList<CNCRackMaterial> getCNCRackMaterial() {
		ArrayList<CNCRackMaterial> materialList = new ArrayList<>();
		String msg = "OK";
		DAO dao = new DAOImpl("com.cncmes.dto.CNCRackMaterial");
		try {
			ArrayList<Object> vos = dao.findAll();
			if (null != vos) {
				for (int i = 0; i < vos.size(); i++) {
					CNCRackMaterial vo = (CNCRackMaterial) vos.get(i);
					materialList.add(vo);
				}
			}
		} catch (SQLException e) {
			msg = "DataUtils getCNCRackMaterial ERR:" + e.getMessage();
		}

...
public class SocketClient {
	...
	public boolean portOfHostOK(String ip, int port){
		boolean ok = false;
		...
		if(null == sdh){
			...
			if(!("".equals(ip) || port <= 1024)){
				...
				try {
					...
					String cnnIP = DebugUtils.getDummyDeviceIP(dummyItem);
					if("".equals(cnnIP)) cnnIP = ip;
					LogUtils.errorLog("SocketClient connect("+getMapKey(ip,port)+") failed:"+e.getMessage()+LogUtils.separator+"Ping("+cnnIP+"):"+NetUtils.pingHost(cnnIP));

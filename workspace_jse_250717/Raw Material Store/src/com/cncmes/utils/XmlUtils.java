...
public class XmlUtils {
	...
	public static void parseSystemConfig() {
		...
		commonCfg.put("sendMail", "0");
		commonCfg.put("smtpServer", "mail1.sto-tech.hk");
		commonCfg.put("port", "465");
		commonCfg.put("username", "xxx@sto-tech.hk");
		commonCfg.put("password", "0000000000");
		commonCfg.put("to", "xxx@sto-tech.hk");
		commonCfg.put("cc", "");
		commonCfg.put("subject", "Raw Material Store Test Message");
		commonCfg.put("content", "This is an email message sent by Raw Material Store System.");
		...
		if (xmlFilePath.startsWith("com/"))
			saveSystemConfig(sysConfig.getDataMap());
	}
			...
			for (String key : config.keySet()) {
				...
				if (null != items && items.size() > 0) {
					...
					for (String item : items.keySet()) {
						...
						if ("socketResponseTimeOut".equals(item))
							cfgItem.addAttribute("unit", "s");
						if ("socketResponseInterval".equals(item))
							cfgItem.addAttribute("unit", "ms");
						if ("DeviceMonitorInterval".equals(item))
							cfgItem.addAttribute("unit", "s");
					}
				}
			}
